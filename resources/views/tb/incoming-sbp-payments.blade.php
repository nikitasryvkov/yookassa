...
<x-layout>
    <div class="flex items-center justify-between mb-6">
        <h1 class="section-title">СБП-платежи</h1>
    </div>
                        ...
                        <div class="flex flex-wrap justify-between gap-4">
                            <div class="space-y-1 text-sm min-w-0 flex-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="badge-info">{{ $payment->brand_name }}</span>
                                </div>
                                <div class="text-xs text-slate-500">Тел: {{ $payment->payer_mobile_number }}</div>
                                <div class="text-xs text-slate-500">Назначение: {{ $payment->purpose }}</div>
                                <div class="text-xs text-slate-500">Merchant: {{ $payment->merchant_id }} &middot; Клиент: {{ $payment->customer_code }}</div>
                                <div class="text-xs text-slate-600">Op: {{ $payment->operation_id }} &middot; QR: {{ $payment->qrc_id }}</div>
